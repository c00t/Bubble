...
unsafe impl<K, V> RcObject for Node<K, V> {
    fn pop_edges(&mut self, out: &mut Vec<Rc<Self>>) {
        out.push(self.left.swap(Rc::null(), Ordering::Relaxed));
        out.push(self.right.swap(Rc::null(), Ordering::Relaxed));
    }
}
    ...
    V: Clone,
{
    ...
    fn seek<'g>(&self, key: &K, cs: &'g Guard) -> SeekRecord<'g, K, V> {
        let r = self.r.load(Ordering::Acquire, cs);
    ...
    fn seek_leaf<'g>(&self, key: &K, cs: &'g Guard) -> SeekRecord<'g, K, V> {
        let r = self.r.load(Ordering::Acquire, cs);
        let s = unsafe { r.deref() }.left.load(Ordering::Acquire, cs);
        let mut leaf = unsafe { s.deref() }.left.load(Ordering::Acquire, cs);
        leaf = leaf.with_tag(Marks::empty().bits());
    ...
    pub fn contains(&self, key: &K, cs: &Guard) -> bool {
        let record = self.seek_leaf(key, cs);
        let leaf_node = unsafe { record.leaf.deref() };
        if leaf_node.key.cmp(key) == cmp::Ordering::Equal {
            true
    ...
    pub fn insert(&self, key: K, value: V, cs: &Guard) -> bool {
        ...
        let mut new_internal = Rc::new(Node {
            key: Key::Inf, // temporary placeholder, it will eventually be max(leaf_key, key)
            value: None,
            left: AtomicRc::null(),
            right: AtomicRc::null(),
        });
    ...
    pub fn remove<'g>(&self, key: &K, cs: &'g Guard) -> Option<Snapshot<'g, Node<K, V>>> {
        ...
        let leaf = loop {
            ...
            match record.leaf_addr().compare_exchange_tag(
                ...
                Err(e) => {
                    ...
                    if record
                        .leaf
                        .ptr_eq(e.current.with_tag(Marks::empty().bits()))
                    {
                        self.cleanup(&record, cs);
                    }
                }
        ...
        loop {
            ...
            if !next_record.leaf.ptr_eq(leaf) {
