//! A multithreading dispatcher with dynamic library support.
...
use crate::runtime::{
    event::Event,
    registry::{SharedRuntimeRegistryBuilder, ThreadRuntimeRef},
...
impl Dispatcher {
    ...
    /// Create the dispatcher with specified number of threads. But with collections of [`Runtime`]
    pub(crate) fn new_impl_runtime_refs(
        mut builder: DispatcherBuilder,
        runtime_refs_builder: SharedRuntimeRegistryBuilder,
    ) -> io::Result<Self> {
        let mut proactor_builder = builder.proactor_builder;
        proactor_builder.force_reuse_thread_pool();
        let pool = proactor_builder.create_or_get_thread_pool();
...
impl DispatcherBuilder {
    ...
    /// Build the [`Dispatcher`], with a [`SharedRuntimeRegistryBuilder`]
    pub fn build_runtime_refs(
        self,
        runtime_refs_builder: SharedRuntimeRegistryBuilder,
