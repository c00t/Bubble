...
use registry::SharedRuntimeRegistry;
use send_wrapper::SendWrapper;

#[cfg(feature = "time")]
use crate::runtime::core::time::{TimerFuture, TimerRuntime};
use crate::runtime::{core::op::OpFlagsFuture, BufResult};

scoped_tls::scoped_thread_local!(static CURRENT_RUNTIME: Runtime);

/// An global runtime registry which set by user when used from different dynamic libraries.
static GLOBAL_SHARED_REGISTRY: OnceLock<Arc<SharedRuntimeRegistry>> = OnceLock::new();

/// Set the global runtime registry
pub fn set_global_registry(registry: Arc<SharedRuntimeRegistry>) {
    if let Err(_) = GLOBAL_SHARED_REGISTRY.set(registry) {
        panic!("set_global_registry called failed");
    }
}

/// Get the global runtime registry
pub fn get_global_registry() -> Arc<SharedRuntimeRegistry> {
...
impl Runtime {
    ...
    pub fn with_current<T, F: FnOnce(&Self) -> T>(f: F) -> T {
        ...
        if CURRENT_RUNTIME.is_set() {
            ...
            if let Some(runtime) = find_runtime() {
                runtime.enter(|| CURRENT_RUNTIME.with(f))
            } else {
    ...
    pub fn poll_with(&self, timeout: Option<Duration>) {
        ...
        let mut entries = heapless::Vec::<usize, 1024>::new();
        let mut driver = self.driver.borrow_mut();
